...
export function TechnicianDashboard() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            >
              <div className="p-0 sm:p-1">
                <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
                  <div className={`w-10 h-10 sm:w-14 sm:h-14 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 ${getPriorityColor(currentTask.priority)}`}>
                    <Wrench className="w-5 h-5 sm:w-7 sm:h-7" />
                  </div>
                  <div className="flex-1 min-w-0 w-full">
                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-1">
                      <span className="font-bold text-sm sm:text-base text-slate-900">{currentTask.id}</span>
                      <span className={`px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-xs font-semibold rounded-full ${getPriorityColor(currentTask.priority)}`}>
                        {currentTask.priority.toUpperCase()}
                      </span>
                    </div>
                    <p className="text-xs sm:text-sm text-slate-500 mb-1">Assigned by {currentTask.assignedBy.name}</p>
                    <h3 className="text-base sm:text-lg font-semibold text-slate-900">{currentTask.type}</h3>
                    <p className="text-xs sm:text-sm text-slate-600 mt-1 line-clamp-2">{currentTask.description}</p>
                    
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-2 sm:mt-4 text-[10px] sm:text-sm text-slate-500">
                      <span className="flex items-center gap-1">
                        <MapPin className="w-3 h-3 sm:w-4 sm:h-4" />
                        <span className="truncate max-w-[120px] sm:max-w-none">{currentTask.address}</span>
                      </span>
                      <span className="flex items-center gap-1">
            ...
            {assignedTasks.length === 0 ? (
              ...
              <div className="divide-y divide-slate-100">
                {assignedTasks.map((wo) => (
                  <div key={wo.id} className="p-2 sm:p-4 hover:bg-slate-50/50 transition-colors">
                    <div className="flex items-start gap-2 sm:gap-4">
                      <div className={`w-9 h-9 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 ${getPriorityColor(wo.priority)}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-1">
                          <span className="font-bold text-xs sm:text-sm text-slate-900">{wo.id}</span>
                          <span className={`px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-xs font-semibold rounded-full ${getPriorityColor(wo.priority)}`}>
                            {wo.priority === 'emergency' ? '🚨 EMERG' : wo.priority.toUpperCase()}
                          </span>
                        </div>
                        <p className="text-[10px] sm:text-xs text-slate-500">{wo.assignedBy.name}</p>
                        <h4 className="font-semibold text-xs sm:text-sm text-slate-900 mt-0.5 sm:mt-1 truncate">{wo.type}</h4>
                        <p className="text-[10px] sm:text-sm text-slate-600 line-clamp-1 sm:line-clamp-2">{wo.description}</p>
                        
                        <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-1.5 sm:mt-2 text-[9px] sm:text-xs text-slate-500">
                          <span className="flex items-center gap-0.5 sm:gap-1">
                            <MapPin className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                            <span className="truncate max-w-[60px] sm:max-w-none">{wo.location}</span>
                          </span>
                          <span className="flex items-center gap-0.5 sm:gap-1">
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                          <button 
                            ...
                            className={`px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold rounded-lg transition-colors flex items-center gap-1 ${
                              wo.priority === 'emergency' 
                          ...
                          <button className="px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm bg-slate-100 text-slate-700 font-medium rounded-lg hover:bg-slate-200 transition-colors flex items-center gap-1">
        ...
        <div className="space-y-6">
          ...
          <SectionCard title="Quick Actions" noPadding>
            <div className="grid grid-cols-2 gap-1.5 sm:gap-2 p-2 sm:p-4">
              <Link href="/dma" className="p-2.5 sm:p-4 bg-slate-50 rounded-lg sm:rounded-xl hover:bg-slate-100 transition-colors text-center block">
                <MapPin className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 mx-auto mb-1 sm:mb-2" />
                <span className="text-xs sm:text-sm font-medium text-slate-700">Field Map</span>
                <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5 sm:mt-1 hidden sm:block">Work locations</p>
              </Link>
              <Link href="/health" className="p-2.5 sm:p-4 bg-slate-50 rounded-lg sm:rounded-xl hover:bg-slate-100 transition-colors text-center block">
                <Radio className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-600 mx-auto mb-1 sm:mb-2" />
                <span className="text-xs sm:text-sm font-medium text-slate-700">Equipment</span>
                <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5 sm:mt-1 hidden sm:block">Sensor status</p>
              </Link>
              <Link href="/ai" className="p-2.5 sm:p-4 bg-slate-50 rounded-lg sm:rounded-xl hover:bg-slate-100 transition-colors text-center block">
                <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-purple-600 mx-auto mb-1 sm:mb-2" />
                <span className="text-xs sm:text-sm font-medium text-slate-700">Knowledge</span>
                <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5 sm:mt-1 hidden sm:block">Repair guides</p>
              </Link>
              <button 
                onClick={() => setShowReportModal(true)}
                className="p-2.5 sm:p-4 bg-slate-50 rounded-lg sm:rounded-xl hover:bg-slate-100 transition-colors text-center"
              >
                <AlertCircle className="w-5 h-5 sm:w-6 sm:h-6 text-red-600 mx-auto mb-1 sm:mb-2" />
                <span className="text-xs sm:text-sm font-medium text-slate-700">Report Issue</span>
                <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5 sm:mt-1 hidden sm:block">Field problems</p>
              </button>
            </div>
          </SectionCard>
      ...
      {/* Footer - Responsive */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between py-2 sm:py-3 px-3 sm:px-5 bg-orange-50 rounded-lg sm:rounded-xl border border-orange-200 gap-2 sm:gap-0">
        <div className="flex flex-wrap items-center gap-2 sm:gap-4">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <User className="w-3 h-3 sm:w-4 sm:h-4 text-orange-600" />
            <span className="text-[10px] sm:text-sm font-medium text-orange-700">Field Tech Active</span>
          </div>
          <div className="hidden sm:block h-4 w-px bg-orange-300" />
          <span className="flex items-center gap-1 text-[10px] sm:text-sm text-slate-600">
            <Signal className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-500" />
            GPS
          </span>
          <span className="flex items-center gap-1 text-[10px] sm:text-sm text-slate-600">
        ...
        <div className="flex items-center gap-2 sm:gap-3 text-[10px] sm:text-sm text-slate-500">
          <span>Emergency: <strong className="text-red-600">211</strong></span>
        </div>
      </div>

      {/* Completion Report Modal - Responsive */}
      {showReportModal && selectedTask && (
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className="bg-gradient-to-r from-emerald-600 to-green-600 p-4 sm:p-5 text-white sticky top-0">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg sm:text-xl font-bold">Complete Task</h2>
                  <p className="text-emerald-100 text-xs sm:text-sm">{selectedTask.id} - {selectedTask.type}</p>
                </div>
                <button onClick={() => setShowReportModal(false)} className="p-1.5 sm:p-2 hover:bg-white/20 rounded-lg">
            ...
            <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-slate-700 mb-1.5 sm:mb-2">Completion Report *</label>
                <textarea
                  value={reportText}
                  onChange={(e) => setReportText(e.target.value)}
                  placeholder="Describe work completed, any issues found, materials used..."
                  className="w-full px-3 sm:px-4 py-2 sm:py-3 border border-slate-200 rounded-lg sm:rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/50 h-24 sm:h-32 resize-none text-sm"
                />
              </div>
              
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-slate-700 mb-1.5 sm:mb-2">Attach Photos</label>
                <div className="border-2 border-dashed border-slate-200 rounded-lg sm:rounded-xl p-4 sm:p-6 text-center hover:border-emerald-500 transition-colors cursor-pointer">
                  <Upload className="w-6 h-6 sm:w-8 sm:h-8 text-slate-400 mx-auto mb-1.5 sm:mb-2" />
                  <p className="text-xs sm:text-sm text-slate-500">Click to upload or drag photos</p>
                  <p className="text-[10px] sm:text-xs text-slate-400 mt-0.5 sm:mt-1">Before/after photos recommended</p>
                </div>
              </div>
              
              <div className="grid grid-cols-2 gap-2 sm:gap-4">
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-slate-700 mb-1 sm:mb-2">Materials Used</label>
                  <input
                    type="text"
                    placeholder="e.g., 2x pipe clamps"
                    className="w-full px-3 sm:px-4 py-1.5 sm:py-2 border border-slate-200 rounded-lg sm:rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-xs sm:text-sm"
                  />
                </div>
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-slate-700 mb-1 sm:mb-2">Time Spent</label>
                  <input
                    type="text"
              ...
              <div className="flex gap-2 sm:gap-3 pt-2 sm:pt-4">
                <button
                  ...
                  className="flex-1 py-2.5 sm:py-3 border border-slate-200 text-slate-700 rounded-lg sm:rounded-xl font-semibold hover:bg-slate-50 transition-colors text-sm"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-2.5 sm:py-3 bg-emerald-600 text-white rounded-lg sm:rounded-xl font-semibold hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1.5 sm:gap-2 text-sm"
                >
