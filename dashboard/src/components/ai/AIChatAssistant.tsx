  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-purple-600 to-blue-600 rounded-full shadow-lg shadow-purple-500/30 flex items-center justify-center text-white hover:scale-110 transition-transform z-50 ${isOpen ? 'hidden' : ''}`}
      >
        <Sparkles className="w-5 h-5 sm:w-7 sm:h-7" />
        <span className="absolute -top-1 -right-1 w-3 h-3 sm:w-4 sm:h-4 bg-emerald-500 rounded-full border-2 border-white animate-pulse" />
      </button>

      {/* Chat Window - Responsive: smaller on mobile, doesn't cover whole screen */}
      {isOpen && (
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 w-[calc(100%-2rem)] sm:w-[380px] md:w-[420px] h-[70vh] max-h-[500px] sm:max-h-[600px] md:h-[650px] md:max-h-[650px] bg-white rounded-2xl shadow-2xl flex flex-col z-50 overflow-hidden border border-slate-200">
          ...
          <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-3 sm:p-4 flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <h3 className="font-bold text-sm sm:text-base flex items-center gap-2">LWSC AI <span className="text-[10px] sm:text-xs bg-white/20 px-1.5 sm:px-2 py-0.5 rounded-full">GPT</span></h3>
                <div className="flex items-center gap-1 sm:gap-2 text-[10px] sm:text-xs text-purple-200">
          ...
          {/* Data Indicators - hidden on very small screens */}
          {systemData && (
            <div className="bg-slate-50 px-2 sm:px-4 py-1.5 sm:py-2 border-b border-slate-200 flex items-center gap-2 sm:gap-4 text-[10px] sm:text-xs overflow-x-auto">
            ...
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[90%] sm:max-w-[85%] ${message.role === 'user' ? 'order-2' : 'order-1'}`}>
                  <div className={`flex items-end gap-1.5 sm:gap-2 ${message.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`px-3 py-2 sm:px-4 sm:py-3 rounded-2xl ${
                      ...
                      {message.isLoading ? (
                        ...
                        <div className={`text-xs sm:text-sm whitespace-pre-wrap ${message.role === 'user' ? '' : 'text-slate-700'}`}>
                    ...
                    <div className="mt-1.5 sm:mt-2 ml-7 sm:ml-9 flex flex-wrap gap-1">
                      {message.suggestions.slice(0, 3).map((suggestion, i) => (
                        <button
                          ...
                          className="px-2 py-1 sm:px-3 sm:py-1.5 bg-white border border-slate-200 rounded-full text-[10px] sm:text-xs text-slate-600 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-colors"
                        >
                          {suggestion}
                        </button>
                      ))}
          ...
          <div className="p-2 sm:p-4 border-t border-slate-200 bg-white">
            <div className="flex items-center gap-1.5 sm:gap-2">
              <input
                ...
                disabled={isTyping}
                className="flex-1 px-3 py-2 sm:px-4 sm:py-2.5 bg-slate-100 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:bg-white transition-colors disabled:opacity-50"
              />
              <button 
                onClick={() => handleSend()}
                disabled={!input.trim() || isTyping}
                className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-purple-600 to-blue-600 rounded-xl flex items-center justify-center text-white hover:opacity-90 transition-opacity disabled:opacity-50"
              >
